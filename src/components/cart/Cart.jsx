...
const Cart = (props) => {
    ...
    const onAddOverlay = async (obj)=>{
        try{
            ...
            if(findOverlay)
            {
            axios.delete(`https://64416e55fadc69b8e08407f5.mockapi.io/cart/${findOverlay.id}`)
            props.setOverlayItems((over) => over.filter(item => item.myId !== obj.myId))
            }
            else{
              const {data} = await axios.post('https://64416e55fadc69b8e08407f5.mockapi.io/cart', obj)
